...
(auto-render), both loaded in `base.html`. Extracted Markdown uses `\(...\)`
for inline LaTeX math and `$$...$$` for display math. Single-dollar inline
math is intentionally not rendered because Brazilian currency values such as
`R$ 250,00` appear frequently in exam text. Rendered output goes into an
